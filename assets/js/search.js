...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (query) {
        ...
        searchContainer.innerHTML = `<p style="color: red; text-align: center; margin-bottom: 8px">Rien à afficher...</p>`;
        searchHeading.innerText = 'Veuillez rechercher quelque chose...';
    }
});
...
function fetchSearchResult(param, idsToToggle) {
    ...
    searchHeading.innerText = `Résultats de recherche pour "${param}"`;
    ...
    fetch(url, options)
        ...
        .then(data => {
            ...
            if (coins.length === 0) {
                coinsList.innerHTML = '<p style="color: red; text-align: center;">Aucun résultat trouvé pour les pièces.</p>';
            }

            if (exchanges.length === 0) {
                exchangesList.innerHTML = '<p style="color: red; text-align: center;">Aucun résultat trouvé pour les échanges.</p>';
            }

            if (nfts.length === 0) {
                nftsList.innerHTML = '<p style="color: red; text-align: center;">Aucun résultat trouvé pour les NFTs.</p>';
            }
